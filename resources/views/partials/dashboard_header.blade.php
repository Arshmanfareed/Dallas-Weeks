            ...
            <script>
                $(document).ready(function() {
                                ...
                                if (elements_array) {
                                    ...
                                    for (var i = 0; i < elements_array.length; i++) {
                                        ...
                                        if (parseInt(left) - subtract < 0) {
                                            left = 0;
                                        } else if (parseInt(left) + $(clone).width() < $('.drop-pad').width()) {
                                            left = parseInt(left) - subtract;
                                        } else {
                                            left = $('.drop-pad').width() - $(clone).width() - step1_top;
                                        }
                                        var top = elements_array[i]['position_y'];
                                        var step1_top = $('#step-1').position().top;
                                        var step1_height = $('#step-1').outerHeight(true);
                                        subtract = parseInt(elements_array[0]['position_y']) - step1_height -
                                            step1_top;
                        ...
                        if (outputElement != null && outputElement.attr('id') != $(this).parent().attr('id')) {
                            ...
                            if (outputElement && inputElement) {
                                ...
                                if (attachInputElement && attachOutputElement) {
                                    ...
                                    var x1 = attachOutputElement.offset().left + (attachOutputElement.outerWidth() / 2);
                                    var y1 = attachOutputElement.offset().top + (attachOutputElement.outerHeight() / 2);
                                    var x2 = attachInputElement.offset().left + (attachInputElement.outerWidth() / 2);
                                    var y2 = attachInputElement.offset().top + (attachInputElement.outerHeight() / 2);
                                    var distance = Math.sqrt(Math.pow((x2 - x1), 2) + Math.pow((y2 - y1), 2));
                                    var extra_left = ($('.drop-pad').outerWidth(true) - $('.drop-pad').width()) / 2;
                    ...
                    function elementProperties(e) {
                        ...
                        var name_html = '';
                        $.ajax({
                            url: "{{ route('getcampaignelementbyid', ':element_id') }}".replace(':element_id',
                                item_id),
                            type: 'GET',
                            dataType: 'json',
                            success: function(response) {
                                if (response.success) {
                                    ...
                                    name_html += '<div class="element_name" data-bs-target="' + item_id + '">' +
                                        list_icon + '<p>' + item_name + '</p></div>';
                                    properties = response.properties;
                                    for (var i = 0; i < properties.length; i++) {
                                        name_html += '<div class="property_item">';
                                        name_html += '<p>' + properties[i]['original_properties'][
                                            'property_name'
                                        ] + ': <br>';
                                        name_html += ' <span style="font-style: italic;">' + properties[i][
                                            'value'
                                        ] + '</span></p>';
                                        name_html += '</div>';
                                    }
                                    ...
                                    name_html += '<div class="element_properties">';
                                    name_html += '<div class="element_name">' + list_icon + '<p>' + item_name +
                                        '</p></div>';
                                    name_html += '<div class="text-center"> Not Found </div></div>';
                                }
            ...
            <script>
                $(document).ready(function() {
                    ...
                    if (current_time >= campaign_time) {
                        ...
                        function placeElement(e) {
                            ...
                            $(document).on('mouseup', function(e) {
                                if (choosedElement) {
                                    ...
                                    var element_y = element.top;
                                    var max_y = element_y + $('.drop-pad').outerHeight() - choosedElement.height();
                                    if (x < element_x && y < element_y) {
                                        choosedElement.css({
                                            left: 0,
                                            top: 0,
                        ...
                        function startDragging(e) {
                            ...
                            $(document).on('mousemove', function(e) {
                                ...
                                if (current_element_id && next_true_element_id) {
                                    if ($('.drop-pad').find('#' + current_element_id + '-to-' +
                        ...
                        message = 'You can not update sequence within a month!';
                    }
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.success) {
                                    ...
                                    name_html += '</div>';
                                    if (message != '') {
                                        name_html += '<div class="text-danger text-center">' + message +
                                            '</div>';
                                    }
                                } else {
                                    name_html += '<div class="element_properties">';
                    ...
                    $('#save-changes').on('click', function() {
                        html2canvas(document.getElementById('capture')).then(function(canvas) {
                            ...
                            $.ajax({
                                url: "{{ route('updateCampaign', ':campaign_id') }}".replace(

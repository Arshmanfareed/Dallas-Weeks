...
class ActionLeadCron extends Command
{
    ...
    {
        $this->logFilePath = storage_path('logs/lead_action.log');
    ...
    {
        file_put_contents($this->logFilePath, ' Working on campaigns started succesfully at ' . now() . PHP_EOL, FILE_APPEND);
        file_put_contents($this->logFilePath, ' {' . PHP_EOL, FILE_APPEND);
        try {
            $current_time = now();
            ...
            foreach ($campaigns as $campaign) {
                ...
                if ($actions->isNotEmpty()) {
                                    ...
                                    switch ($element['element_slug']) {
                                        case 'view_profile':
                                            if ($view_count < $view_distribution_limit) {
                                                ...
                                                if ($success) {
                                                    file_put_contents($this->logFilePath, '         Profile Viewed successfully' . PHP_EOL, FILE_APPEND);
                                                }
                                                $view_count++;
                                            } else if ($view_distribution_limit == 0) {
                                                file_put_contents($this->logFilePath, '         Profile View Limitation reached' . PHP_EOL, FILE_APPEND);
                                                break 2;
                                            }
                                            break;
                                        case 'invite_to_connect':
                                            if ($invite_count < $invitation_distribution_limit) {
                                                ...
                                                if ($success) {
                                                    file_put_contents($this->logFilePath, '         Invitation to connect sent successfully' . PHP_EOL, FILE_APPEND);
                                                }
                                                $invite_count++;
                                            } else if ($invitation_distribution_limit == 0) {
                                                file_put_contents($this->logFilePath, '         Invitation Limitation reached' . PHP_EOL, FILE_APPEND);
                                                break 2;
                                            }
                                            break;
                                        case 'message':
                                            if ($message_count < $message_distribution_limit) {
                                                ...
                                                if ($success) {
                                                    file_put_contents($this->logFilePath, '         Message sent successfully' . PHP_EOL, FILE_APPEND);
                                                }
                                                $message_count++;
                                            } else if ($message_distribution_limit == 0) {
                                                file_put_contents($this->logFilePath, '         Message Limitation reached' . PHP_EOL, FILE_APPEND);
                                                break 2;
                                            }
                                            break;
                                        case 'inmail_message':
                                            ...
                                            if ($success) {
                                                file_put_contents($this->logFilePath, '         Inmail message sent successfully' . PHP_EOL, FILE_APPEND);
                                            }
                                            break;
                                        case 'follow':
                                            $success = $cc->follow($action, $account_id);
                                            if ($success) {
                                                file_put_contents($this->logFilePath, '         Follow user successfully' . PHP_EOL, FILE_APPEND);
                                            }
                                            break;
                                        case 'email_message':
                                            $success = $cc->email_message($action, $account_id, $element, $campaign_element);
                                            if ($success) {
                                                file_put_contents($this->logFilePath, '         Email sent successfully' . PHP_EOL, FILE_APPEND);
                                            }
                                            break;
                                        case 'if_connected':
                                            $conditional_output = $cc->if_connected($action, $account_id);
                                            if ($conditional_output == 'true') {
                                                file_put_contents($this->logFilePath, '         Lead is already connected' . PHP_EOL, FILE_APPEND);
                                            } else {
                                                file_put_contents($this->logFilePath, '         Lead is not connected' . PHP_EOL, FILE_APPEND);
                                            }
                                            $success = true;
                                            break;
                                        case 'if_email_is_opened':
                                            ...
                                            if ($conditional_output == 'true') {
                                                file_put_contents($this->logFilePath, '         Email is already opened' . PHP_EOL, FILE_APPEND);
                                            } else {
                                                file_put_contents($this->logFilePath, '         Email is not opened' . PHP_EOL, FILE_APPEND);
                                            }
                                            $success = true;
                                            break;
                                        ...
                                        case 'if_has_verified_email':
                                            ...
                                            if ($conditional_output == 'true') {
                                                file_put_contents($this->logFilePath, '         Email is already verified' . PHP_EOL, FILE_APPEND);
                                            } else {
                                                file_put_contents($this->logFilePath, '         Email is not verified' . PHP_EOL, FILE_APPEND);
                                            }
                                            $success = true;
                                            break;
                                        case 'if_free_inmail':
                                            ...
                                            if ($conditional_output == 'true') {
                                                file_put_contents($this->logFilePath, '         Inmail is already free' . PHP_EOL, FILE_APPEND);
                                            } else {
                                                file_put_contents($this->logFilePath, '         Inmail is not free' . PHP_EOL, FILE_APPEND);
                                            }
                                            $success = true;
                                            break;
                                        default:
                                            file_put_contents($this->logFilePath, '         Unknown action ' . $element['element_slug'] . ' of campaign # ' . $campaign['id'] . PHP_EOL, FILE_APPEND);
                                            break;
                                    }
                                }
                            } else {
                    ...
                    $fileContent = '   No actions found for campaigns # (' . $campaign['id'] . ')' . PHP_EOL;
                    file_put_contents($this->logFilePath, $fileContent, FILE_APPEND);
                }
            }
        } catch (\Exception $e) {

...
class isIntegrationAllowed
{
    ...
    public function handle(Request $request, Closure $next)
    {
        try {
            ...
            if ($data['manage_linkedin_integrations'] == true || $data['manage_linkedin_integrations'] == 'view_only') {
                return $next($request);
            }
            /* If the user does not have permission, throw an exception */
            throw new Exception('You can not access integrations', 403);
        } catch (Exception $e) {
            Log::info($e);

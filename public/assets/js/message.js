...
$(document).ready(function () {
    ...
    intervalId = setInterval(function () {
        const isAnyAjaxInProgress = (getMessageAjax || getReceiverAjax || updateChatListAjax.length > 0 || getChatAjax);
...
function getMessages() {
    ...
    if (chat_id != '') {
        const data_chat = $('#chat-message').attr('data-chat');
        if (chat_id !== data_chat) {
            if (updateChatListAjax.length > 0) {
                const ajaxRequests = [...updateChatListAjax];
                ajaxRequests.forEach((ajax, index) => {
                    if (ajax && ajax.readyState !== 4) {
...
function getChatData() {
    ...
    chats.each(function () {
        var chat = $(this);
        if (
            chat.find('.skel_chat_img').length > 0 &&
            chat.find('.skel_chat_name').length > 0 &&
            chat.find('.skel_latest_message').length > 0 &&
            chat.find('.skel_latest_message_timestamp').length > 0
        ) {
        ...
        success: function (response) {
            if (response.success) {
                ...
                if (response.message?.length) {
                    ...
                    receivers[chat.attr('data-profile')] = profile;
                    chat.prop('id', '');
        ...
        success: function (response) {
            if (response.success && response.message?.length) {
                ...
                if (latestMessage.text) {
                    ...
                    chat.removeClass('skel_latest_message');
                }
                if (latestMessage.timestamp) {
                    const formattedDate = new Date(latestMessage.timestamp).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' });
                    chat.find('.latest_message_timestamp').html(formattedDate).removeClass('skel_latest_message_timestamp');
                } else {
                    chat.removeClass('skel_latest_message_timestamp');
                }
            } else {
                chat.find('.latest_message').removeClass('skel_latest_message');
                chat.find('.latest_message_timestamp').removeClass('skel_latest_message_timestamp');
...
function updateMessageLoader(e) {
    ...
    const chat_id = $('#chat-message').attr('data-chat');
...
function getLatestMessageInChat() {
    const chatId = $('#chat-message').attr('data-chat');
    const count = $('#chat-message>ul>li').length || 0;
    if (!chatId) return;
